...
\graphicspath{ {./images/} }
\title{{\Large{\textbf{Probabilistic Methods in Stochastic Control}}}}
\author{Wei Chen}
\date{\today}
...
\begin{document}
    ...
    \begin{center}
		\large\textbf{Preface}
    \end{center}~\
    This is the reading note of the book \textit{Continuous-time Stochastic Control with Applications in Finance} by Huyên Pham and the book \textit{Backward Stochastic Differential Equations: From Linear to Fully Nonlinear Theory}. The note is written by the author with the support of ChatGPT. Assuming background knowledge in probability theory and basic stochastic calculus, this note introduces the basic notions of stochastic control and the BSDE approach as the main probabilistic method. To be more specific, this note covers chapters 1, 2, 3, and 6 of the first book and chapters 4, 5, 7, 8 of the second book. In addition, this note is also supplemented by the book \textit{Lectures on BSDEs, Stochastic Control, and Stochastic Differential Games with Financial Applications} by René Carmona (on the section about the stochastic Pontryagin maximum principle). Topics of the note include: preliminaries in stochastic control problems, the classical PDE approach, the BSDE approach, basics of BSDE theory, and two types of special BSDEs: BSDEs with quadratic growth in $Z$ and forward-backward differential equations. 
	\allowdisplaybreaks
	\pagenumbering{roman}
	\setcounter{page}{1}

...
\begin{notetheorem}[2.3 (cancellation)]
...
\begin{notetheorem}[2.4 (linear congruences)]
...
\begin{notetheorem}[2.5 (Chinese remainder theorem)]
...
\begin{notetheorem}[2.7 (Fermat's little theorem)]
If $p$ is prime and $p\nmid a$, then
\[
 a^{p-1}\equiv1\pmod p.
\]
...
\begin{notetheorem}[2.8 (Euler's theorem)]
If $\gcd(a,m)=1$, then
\[
...
\begin{notetheorem}[2.10 (Freshman's dream modulo a prime)]
...
\begin{notetheorem}[2.11 (Wilson's theorem)]
If $p$ is prime, then
\[
 (p-1)!\equiv-1\pmod p.
\]
...
\begin{notetheorem}[2.13 (converse to Wilson)]
Let $n\ge2$.  If
\[
 (n-1)!\equiv-1\pmod n,
\]
...
\begin{noteremark}[(trial division)]
...
\begin{noteapplication}[2.1 (Fermat primality test)]
...
\begin{noteapplication}[2.2 (Miller--Rabin primality test)]
...
\begin{notetheorem}[2.15 (Rabin's bound)]
...
\begin{notetheorem}[2.16 (Hensel's lemma, simple root)]
...
\begin{notetheorem}[2.17 (singular-root alternatives)]

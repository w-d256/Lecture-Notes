...
Throughout the complex case, inner products are taken linear in the first
variable and conjugate linear in the second.  All formulas below use this
convention.
...
\begin{proof}
For each fixed $s\in S$, the map $x\mapsto\inner{x}{s}$ is continuous and
linear (or conjugate linear, according to the chosen convention).  Hence
\[
  S^\perp=\bigcap_{s\in S}\Ker\bigl(x\mapsto\inner{x}{s}\bigr)
\]
is a closed linear subspace.  Orthogonality to $S$ is equivalent, by linearity
and continuity, to orthogonality to $\overline{\Span S}$, proving the first
identity.

Put $M=\overline{\Span S}$.  Clearly $M\subseteq M^{\perp\perp}$.  If
...
\[
  \inner{x}{x-P_Mx}=\|x-P_Mx\|^2\ne0.
\]
Hence $x\notin M^{\perp\perp}$ and $M^{\perp\perp}=M$.  Finally, if
$x\in S\cap S^\perp$ and $S$ is a subspace, then
$\|x\|^2=\inner{x}{x}=0$, so $x=0$.
\end{proof}
...
\begin{proof}
Apply the projection theorem to the closed convex set $M$.  For $x\in H$, let
$y=P_Mx$.  Since $M$ is a linear subspace, the variational inequality may be
tested with $y\pm v$ and, over $\mathbb C$, with $y\pm iv$, where $v\in M$.
It follows that
\[
  \inner{x-y}{v}=0\qquad(v\in M),
\]
so $x-y\in M^\perp$.  Hence $H=M+M^\perp$.

The sum is direct: if $z\in M\cap M^\perp$, then
$\|z\|^2=\inner{z}{z}=0$.  Thus the decomposition is unique.  Equivalently,
if $x=y_1+z_1=y_2+z_2$ with $y_i\in M$ and $z_i\in M^\perp$, then
$y_1-y_2=z_2-z_1\in M\cap M^\perp=\{0\}$.
\end{proof}
...
sesquilinear form, linear in the first variable and conjugate linear in the
second.
The induced norm is
\[
  \|x\|=\sqrt{\inner{x}{x}}.
...
\begin{proof}
Apply the theorem on mutually orthogonal subspaces to
$E_n=\Span\{e_n\}$.  The orthogonal projection onto $E_n$ is
\[
  P_nu=\inner{u}{e_n}e_n
\]
for the convention in which the first variable is linear.  Since the closed
linear span of the $e_n$ is all of $H$, the preceding projection theorem gives
\[
  u=\sum_{n=1}^\infty P_nu
   =\sum_{n=1}^\infty\inner{u}{e_n}e_n
\]
in norm, and its norm identity is precisely Parseval's formula.

Conversely, let $(a_n)\in\ell^2$ and put $v_n=a_ne_n$.  The vectors $v_n$ are
orthogonal and
\[
  \sum_n\|v_n\|^2=\sum_n|a_n|^2<\infty.
\]
The orthogonal-series lemma therefore gives a vector
$u=\sum_na_ne_n\in H$ and
\[
  \|u\|^2=\sum_n|a_n|^2.
\]
Continuity of the inner product and orthonormality yield
$\inner{u}{e_k}=a_k$.  If another vector has the same coefficients, its
difference from $u$ is orthogonal to every $e_k$ and hence to their dense
linear span; the orthogonal density criterion makes that difference zero.
\end{proof}
...
\begin{proof}
Let $(x_n)$ be a countable dense subset of $H$.  Starting with the first
nonzero $x_n$, discard each later vector that already belongs to the span of
the retained predecessors.  This leaves a finite or countable linearly
independent sequence $(y_n)$ whose linear span contains every $x_n$ and is
therefore dense in $H$.

Apply Gram--Schmidt:
\[
  z_n=y_n-\sum_{k<n}\inner{y_n}{e_k}e_k,
  \qquad
  e_n=\frac{z_n}{\|z_n\|}.
\]
Linear independence ensures $z_n\ne0$.  Inductively, the $e_n$ are
orthonormal and
\[
  \Span\{e_1,\ldots,e_n\}
  =\Span\{y_1,\ldots,y_n\}.
\]
Thus their total linear span is dense in $H$, so they form an orthonormal
basis.
\end{proof}
